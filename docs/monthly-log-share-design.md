...
カルーセル順は固定。内部マスターは透明背景の 9:16・1080×1920 で描画する。外部へ渡す PNG は、アルファ境界の外側を切り詰めた可変サイズ（四辺に 24 物理ピクセルの透明余白）。参考画像は約 3:5。マスターの配置は 9:16 キャンバスのまま変えず、出力を 9:16 へ戻す余白は足さない。X など写真の上へ重ねる素材として使い、位置とサイズは各 SNS の編集画面で調整する。
...
- 内部マスターは 1080×1920・透明。4テンプレートとも同じクロップ処理を通す
- 外部出力は描画ピクセル（alpha > 0。半透明の縁・影を含む）の最小矩形＋上下左右 24px。元画像の端を超える分はクランプ。テンプレートごとに寸法は違ってよい
- 完全透明マスターは出力せず失敗とする
...
- 閉じるボタン
- 市松模様を背景にした透明 PNG プレビュー。カード外形は固定 9:16 のまま。トリミング済み PNG をその枠内へ中央・aspectFit で置く（出力の縦横比は変形しない）
...
`MonthlyLogShareRenderer` が SwiftUI テンプレートを `ImageRenderer` でマスター描画し、`MonthlyLogShareImageTrimmer` でクロップする。

- 論理サイズ 360×640pt
- `scale = 3`
- マスター画素 1080×1920（`masterPixelSize`）
- `isOpaque = false`
- 背景は透明のまま
- エクスポートはマスターを再描画せず、アルファ境界＋24px で切り出した可変サイズ PNG
- コピー・写真保存・iOS 共有シートは同じトリミング済み PNG Data を使う
...
- `MonthlyLogShareRendererTests` — 4テンプレート、内部マスター 1080×1920、最終 PNG はマスター以下の可変サイズ、透明保持、少なくとも一方向の余白除去、描画の外側に 24px、金額は template 1 と 4 のみ、カレンダーは 1〜3 のみ、表紙なし、0冊、月名・曜日がアプリ言語に依存しない、4〜6行の共有セルが 2:3、LINE Seed の計測と描画の一致
