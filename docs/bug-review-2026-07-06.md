...
| D-4 | 中 | ✅ 修正済み (2026-07-07) | 同一日に複数冊登録した場合、カレンダーから開けるのは最新 1 冊のみ（+N バッジは出るが他の本への導線がない）。→ `DaySelection` ラッパ＋`@State selectedDay`＋`.sheet(item:)` を追加。複数冊の日セルは `Button` で一覧シート（`NavigationStack`＋`List`・各本への `NavigationLink`・`.presentationDetents([.medium, .large])`）を提示、1冊の日は従来どおり直接遷移（`ca60088`）<br>**後日変更 (2026-08-12)**: 複数冊一覧は引き続きシートで表示するが、行選択時は詳細を一覧シート内へpushせず、一覧シートを閉じる→dismiss完了後に親のNavigationStackから通常の詳細画面を開く方式へ変更。書籍詳細のボトムシートを物理画面最上端まで展開可能にするための変更（シート内のNavigationStackでは最上端まで展開できないため） | `Views/BookshelfCalendarView.swift` `dayCell` / `dayBooksSheet` |
