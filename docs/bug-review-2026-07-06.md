...
更新日: 2026-07-09（R2完了・B-4/D-3 の状態同期／F-4 手動書影の共有非反映を追加・R6保留／F-1・F-2 を「iOS側完了・Web側のみ残」の中間状態に明確化）
...
| # | 優先度 | 状態 | 内容 | 場所 |
|---|--------|------|------|------|
| F-1 | 高 | ⬜ Web側のみ残（iOS側は2026-07-07完了） | 共有ページが価格を最小通貨単位のまま「円」で表示（$12.99＝1299 が「1,299円」）。根本原因は共有 API のペイロードに通貨コードが無いこと。iOS 側 `ShareBookItem` と Web 側の両方の修正が必要。**iOS 側は完了**（`0c343bb`：共有ペイロードに本ごとの `currencyCode` と表示通貨換算の合計 `totalValue`/`totalCurrencyCode` を追加）。**Web 側が未着手**（`app/share/[id]/page.tsx` で通貨コードを読んで正しく通貨表示する処理が残る） | iOS: `Services/ShareService.swift`（✅済）／ Web: `app/share/[id]/page.tsx`（⬜残） |
| F-2 | 中 | ⬜ Web側のみ残（iOS側は2026-07-07完了） | Redis 書き込みが失敗しても共有 URL を 200 で返すため、リンク先が 404 になる。**iOS 側は失敗時の汎用エラー表示を追加済み**（`b7c7a83`：症状緩和＝ユーザーへのフィードバック）。**本丸の Web 側が未着手**（書き込み失敗時に非200を返す根本修正が残る） | iOS: `Views/ReadingListDetailView.swift`（✅エラー表示済）／ Web: `app/api/lists/route.ts` ＋ `lib/redis.ts`（⬜残・本丸） |
...
4. **F-1 / F-2** 共有の通貨対応と Redis エラー処理（**iOS 側は 2026-07-07 完了・Web 側 `bookbank-share` のみ残**。優先度高）
