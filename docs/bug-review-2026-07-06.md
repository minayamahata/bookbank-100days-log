...
| D-4 | 中 | ✅ 修正済み (2026-07-07) | 同一日に複数冊登録した場合、カレンダーから開けるのは最新 1 冊のみ（+N バッジは出るが他の本への導線がない）。→ `DaySelection` ラッパ＋`@State selectedDay`＋`.sheet(item:)` を追加。複数冊の日セルは `Button` で一覧シート（`NavigationStack`＋`List`・各本への `NavigationLink`・`.presentationDetents([.medium, .large])`）を提示、1冊の日は従来どおり直接遷移（`ca60088`）<br>**後日変更 (2026-08-13)**: 複数冊一覧は引き続きシートで表示するが、行選択時は詳細を一覧シート内へpushせず、一覧シートを閉じる→dismiss完了後に親のNavigationStackから通常の詳細画面を開く方式へ変更。書籍詳細のボトムシートを物理画面最上端まで展開可能にするための変更（シート内のNavigationStackでは最上端まで展開できないため）<br>**視覚補正 (2026-08-17)**: 同日複数冊一覧の行を「表紙｜書名・著者｜右端に金額」へ変更。行内要素は表紙に対して縦中央。日付はシートタイトル（yyyy.MM.dd）にあるため行内では繰り返さない。dismiss後pushの挙動は変更しない | `Views/BookshelfCalendarView.swift` `dayCell` / `dayBooksSheet` |
...
| # | 優先度 | 状態 | 内容 | 場所 |
|---|--------|------|------|------|
| F-1 | 高 | ✅ 完了 (2026-07-09) | 共有ページが価格を最小通貨単位のまま「円」で表示（$12.99＝1299 が「1,299円」）。根本原因は共有 API のペイロードに通貨コードが無いこと。**iOS 側完了**（`0c343bb`：共有ペイロードに本ごとの `currencyCode` と表示通貨換算の合計 `totalValue`/`totalCurrencyCode` を追加）。**Web 側完了**（`bookbank-share` `9b27bb5`：`page.tsx` の `formatMoney` が本ごと `currencyCode`・合計 `totalCurrencyCode` を読んで記号・桁を出し分け・未指定はJPY後方互換）。2026-07-09 に Web 側を `origin/main` へ push・Vercel 自動デプロイで本番反映 | iOS: `Services/ShareService.swift`（✅）／ Web: `bookbank-share` `app/share/[id]/page.tsx`（✅） |
| F-2 | 中 | ✅ 完了 (2026-07-09) | Redis 書き込みが失敗しても共有 URL を 200 で返すため、リンク先が 404 になる。**iOS 側完了**（`b7c7a83`：失敗時の汎用エラー表示）。**Web 側完了・本丸解決**（`bookbank-share` `2038d65`：`redis.set` が失敗/未設定時 `false` を返し、`app/api/lists/route.ts` が list 書き込み失敗時に 500 を返して壊れた共有URLを渡さない。mapping はベストエフォートと切り分け）。2026-07-09 に Web 側を `origin/main` へ push・Vercel 自動デプロイで本番反映 | iOS: `Views/ReadingListDetailView.swift`（✅）／ Web: `bookbank-share` `app/api/lists/route.ts` ＋ `lib/redis.ts`（✅） |
...
## グループH: リリース後クラッシュレポート（v1.5.0・2026-07-19 分析）

v1.5.0 リリース1週間後の Xcode Organizer クラッシュレポート3件の分析結果（2026-07-19 実施・オーナー承認済み）。3件とも各1レポートの単発で、**R3（UUID移行・データ層）に起因する証拠はなし**。マイグレーション失敗系（起動時クラッシュ・データ消失）の報告がゼロであることをもって R3 の本番浸透を確認し、R4 開始を承認した。v1.5.1 パッチは不要。

| # | 優先度 | 状態 | 内容 | 場所 |
|---|--------|------|------|------|
| H-1 | 中 | ⬜ R4で解消（構造的対策） | **SwiftData: 削除済みモデルへのアクセスでクラッシュ**。`_KKMDBackingData.getValue<A>(forKey:)` の assertion failure（クラッシュポイント `CTwXFveyAZl6Ax7rDQx2L0`・2026-07-13・iOS 26.5）。`EditBookView` のシート表示トランジション中に body が評価され、`isManual` → `UserBook.source` getter で削除済み `@Model` のバッキングデータを読んだ。再現条件（推定）は `UserBookDetailView` の削除確定（`context.delete`→`dismiss()`）と編集シート提示のレース。根本原因は `@Bindable` で生モデルをViewが保持する R3 以前からの構造で、**R4 の DTO 化が恒久対策**（詳細は `docs/r4-repository-abstraction-notes.md` 8.4節）。R4 完了後に Organizer で同一シグネチャの再発ゼロを確認する | `Views/EditBookView.swift` `isManual`（35行）／ `Views/UserBookDetailView.swift` `deleteBook` と編集シートの競合 |
| H-2 | 低 | 👁 静観・監視 | **SwiftUI Environment assertion**（クラッシュポイント `C9bBuXwf_YMfcIf3k-6MCV`・2026-07-16・iOS 26.5）。ナビゲーション遷移中に UIKit の `NavigationButtonBar` が SwiftUI 製ツールバー項目（`UIKitBarItemHost`）をサイズ計測する経路で、`@Environment(XxxManager.self)`（レジスタ痕跡から `LanguageManager` / `CurrencyManager` / `ThemeManager` のいずれか）が環境未注入のまま読まれ即トラップ。iOS 26 の environment 伝播の穴を踏んだ可能性が高く、アプリのコードフレームなし。**防御策（`@Environment(Type.self)` のオプショナル取得）は R4 に含めず、再発が積み上がった時点で判断**（オーナー決定 2026-07-19） | アプリ側の特定箇所なし（toolbar＋Observable environment の一般構造） |
| H-3 | 低 | 👁 静観・監視 | **malloc freelist 破損の検知トラップ**（クラッシュポイント `D4gY1-GE9tM9bzdztuZHWa`・2026-07-16・iOS 26.3.1・iPhone 13 mini）。SF Symbols 描画（`CUINamedVectorGlyph`→CoreSVG）中のヒープ確保時に xzone malloc がフリーリスト破損を検知。破損自体はそれ以前にプロセス内のどこかで発生しており検知地点＝原因地点ではない。再現条件の特定は事実上不可能。同一シグネチャが複数件積み上がらない限りアクション不能 | 特定不能（環境依存・アプリのコードフレームなし） |
