...
作成日: 2026-08-17
更新日: 2026-08-19（5言語訳レビューと受け入れをオーナーOKでクローズ。R4.7専用の移行前バックアップ・日付整合・編集の一括保存を追加。実機での旧スキーマ→新スキーマ移行確認はオーナーがOKのまま）／2026-08-18（通帳の再読行は「再読」の左にテーマ色の丸＋`icn_repeat`。読書履歴の●と縦棒は透過なしの `Color.primary`。同日の再読は1件ずつ消す。詳細の「再読を記録」を削除。追加は検索、日付変更・削除は編集画面。読書履歴は詳細情報の上の別枠、初回表記は登録日）／2026-08-17（R4.7 / v1.7.0 初版）
