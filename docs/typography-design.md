...
集計の年見出し（34pt Bold・上8pt・下16pt）は変えない。日本語以外でカード見出しが切れて見えたのは、年別 `TabView` が 628pt 固定のまま `YearlyChartContent` が高くなり、中央配置で上下が欠けたため。内容の自然高を測って上揃えし、`TabView` の高さだけ追従する。
...
- 繁体字 UI の行高（集計の年別ページは 2026-08-19 に `TabView` 高さを内容の実測へ追従。年見出しのサイズ・余白は未変更。他画面は未確認）
