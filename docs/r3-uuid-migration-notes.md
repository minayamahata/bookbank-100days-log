...
| 9 | 移行前バックアップの保持期間 | 整合性検証（4.5節②）の通過後に**即削除**。検証通過＝移行成功が確認済みのため保全対象がない。バックアップはDBファイルのみ（数MB規模）だが、不要になった時点で残さない | 本書 4.5節① |
| 10 | バックアップ不能時（空き容量不足等）の挙動 | バックアップを**スキップして通常起動**。バックアップは追加の防御層であり、それ自体が起動不能の原因になってはならない（スキップはOSLogに記録する）。ただしバックアップ対象をDBファイルのみ（数MB規模）に限定したため、このスキップは事実上ほぼ発生しない | 本書 4.5節① |
...
- **コピー対象**: `default.store`・`-shm`・`-wal` の**DBファイルのみ**。実パスは Application Support 配下。**書影（`.externalStorage` の `coverImageData`）は対象外**——移行（スキーマ変更・UUIDバックフィル・並び順変換）が書き換えるのはDBだけで書影は移行中に不変であり、保護不要なため。これにより必要容量が数MB規模に収まり、容量不足によるスキップ（前提10）が事実上発生しなくなる。externalStorage をコピー対象にも復元削除対象にも含めないことで、復元時に画像を削除する事故が構造的に起きない（単一の `storeFileItems` を共用）
