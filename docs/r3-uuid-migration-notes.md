...
作成日: 2026-07-09
更新日: 2026-07-09（データ破壊ゼロの追加要件＝4.5節（移行前バックアップ・整合性検証・実機検証）を追記）
...
| 順 | 内容 | 対象 | 理由・リスク |
|----|------|------|-------------|
| 1 | 移行前バックアップ＋復元リトライ経路＋空き容量チェック（4.5節①） | `BookBankApp.swift`（`ModelContainer` 生成まわり）＋新規バックアップユーティリティ | **スキーマ変更より先に防御層を入れる**。この時点ではスキーマ未変更のため、バックアップ・復元の経路を単独で検証できる |
| 2 | 4モデルへの `uuid` 追加（フィールド＋init明示代入）＋ `UUIDBackfillMigration` 新設（整合性検証②の件数・uuid充足チェック込み）＋一意性・検証判定のユニットテスト | `Models/Passbook.swift` / `UserBook.swift` / `ReadingList.swift` / `MonthlyMemo.swift`＋新規 `Utils/`（または `Services/`）のマイグレーションファイル＋`BookBankApp.swift`（フック追加） | スキーマ変更を伴う唯一のステップ。**軽量マイグレーションが通ることを、旧バージョンのストアを持つシミュレータ/実機で最初に確認**してから先へ進む |
| 3 | `ReadingList.bookIds` 追加・`orderedBooks` / `saveBookOrder` のuuid化・`ReadingListOrderMigration` 新設（並び順一致検証②(c)込み）・`stableID` 削除＋変換ロジックのユニットテスト | `Models/ReadingList.swift`＋マイグレーションファイル | **最大のリグレッションポイント**（リストの並び順消失）。変換前後で `orderedBooks` の結果が一致することをテストで縛る |
| 4 | `xcodebuild test` 全通し＋シミュレータでのアップデートシナリオ確認。**実機TestFlight検証（4.5節③）は人間タスクとして明示報告** | - | 完了条件（ロードマップR3）: 既存データ入りの実機アップデートでデータ無損失・リスト並び順維持 |
| 5 | ドキュメント同期（ロードマップR3の完了マーク・本書のズレ修正） | `docs/` | 実装ガイド 4.5節 |
