...
作成日: 2026-07-07
更新日: 2026-08-17（R4.7: 1.1節に「意図的な再読は許可、accidental duplicate は引き続き防止」を追記。再読の正は `docs/reread-history-spec.md`）／2026-08-13（R4.5 ステップ8'の変更を反映——0件時のボタン文言を既存キー `book.register`「本を登録する」へ変更し `bookshelf.search.online_cta` を削除・検索モード中は右下の＋ボタンを隠す・検索モードにつながり一覧と橋渡しチップが同乗〈仕様は `docs/memo-tagging-design.md` 4.7節が正〉）／2026-07-08（実装確定に伴い 3.1/3.2/3.4/5/7/8 を更新）
...
**R4.7追記**: 書籍検索（`BookSearchView`）では、意図的な再読は許可する。登録済み行のタップは新しい本を作らず、既存本へ再読履歴を足す。accidental duplicate は引き続き防止する。本棚内検索の結果タップは従来どおり既存本の詳細へ進むだけで、再読の追加先ではない。
