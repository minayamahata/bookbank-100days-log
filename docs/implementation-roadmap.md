...
作成日: 2026-07-07
更新日: 2026-08-07（**共有IDのuuid化を R4.5 に含めることをオーナーが確定**——R4申し送り 10.1。サーバ側がID形式に依存していないことをオーナーが確認し、クライアント1行＋後片付けで済むことが確定した。メモタグT1とはコミットを分ける）／2026-08-07（**R4完了**——移行Phase 1〈リポジトリ抽象化〉の全7ステップが終了し、実機での全画面突合と目視スモークをオーナーが完了。完了マーク・完了条件・申し送り〈設計メモ第10章〉への参照を追加し、実施順とネクストアクションを R4.5 起点へ更新。**R4.5 着手時の最初の判断は共有IDのuuid化を含めるかどうか**）／2026-07-19（**Web優先へ全面改訂・オーナー確定**: Web v1をR6直後に前倒し・R9はWeb v1後に順次化・R6にエンタイトルメント同期を追加・R4.5メモタグ新設・R5ノードはβ→本リリース方式でWeb後へ・N0はラベル付け完了後にクローズし凍結）
...
| `docs/memo-tagging-design.md` | T1〜T2（メモタグ） |
...
flowchart LR
    ...
    R4 --> R45[R4.5 v1.6.x<br/>メモタグ T1 + UI改善]
...
2. **R4.5 v1.6.x** — メモタグ T1 + UI改善 + **共有IDのuuid化**（小リリース）。**次はここ**
...
### R4.5: v1.6.x — メモタグ T1 + UI改善（新設・2026-07-19）

**設計メモは `docs/memo-tagging-design.md`**（ドラフト・オーナーレビュー待ち）。ユーザーが**意図して本をつなげられる**軽量記法（`#タグ`）をメモに導入する。

**前提条件（確定）**: タグは**メモ文字列内の記法でありスキーマ変更ゼロ**（`UserBook.memo` / `MonthlyMemo.text` は String のまま）。**R6のクラウド移行と干渉しない**（移行対象はプレーン文字列のまま・移行ウィザードの変更不要）。

| 項目 | 実装内容 |
|------|---------|
| T1: タグ記法 | `#タグ` のパース（正規表現ベース・NLP不要＝日韓の品詞タグ非対応の影響を受けない）。正規化はノードと同じNFKC＋小文字 |
| T1: 入力・表示 | メモエディタの入力補助（`#` 入力で既存タグをサジェスト）・メモ表示のタグハイライト・書籍詳細のタグチップ |
| T1: タグ活用 | 本棚のタグ絞り込み (仮・設計メモで確定) |
| **共有IDのuuid化** ✅ 実施決定 (2026-08-07) | R4申し送りからの持ち込み（`docs/r4-repository-abstraction-notes.md` 10.1）。`ShareService` が送る `readingListId` を `legacyShareId`（`persistentModelID` 文字列）から `uuid` へ。**再起動をまたぐと再共有のたびに別URLが発行される既存不具合の解消**。サーバ側の変更は不要（ID形式に依存していないことをオーナーが確認済み）。あわせて `ReadingListDTO.legacyShareId` と `ModelDTOMapping` の該当行を削除し、アプリ本体から `persistentModelID` の使用を消す。**メモタグT1とはコミットを分ける**。リリースノートは「共有リンクは常に最新の内容を表示します」に統一 |
| UI改善 | バックログから選定（着手時に確定） |

> T2（タグ共有エッジ＝ノードグラフとの連携・重み付け）は R5 ノードβに含める。**タグを先に出すことで、ノードβ到来時にはユーザーのタグデータが蓄積済み**＝グラフ初日から「意図したつながり」が表示される順序を作る。
...
| T2: タグエッジ | R4.5のタグを新しいエッジ軸として統合（重みは順序のみ確定: シリーズ1.0 > タグ > 著者0.6。具体値はノードβの実データで調整・ストップワード/IDF非適用・reasons表示「#〇〇」）。`bookIndex.keywords` に `source: "tag"` を追加 | `docs/memo-tagging-design.md` |
...
4. **R4.5（v1.6.x）の着手**: メモタグ T1 + UI改善 + 共有IDのuuid化（✅ 実施決定・2026-08-07・R4申し送り 10.1）。**着手の前提は `docs/memo-tagging-design.md`（ドラフト）の (仮) 項目確定＝T0**。設計メモ側の (仮) が残ったままではT1の実装に入れない
