...
| D-4 ✅ | カレンダー同日複数冊: 日セルタップで複数冊リスト（シートまたはポップアップ）を出す。→ 完了 (2026-07-07)・`DaySelection`＋`.sheet` で一覧シート提示（`ca60088`） | `Views/BookshelfCalendarView.swift` `dayCell` / `dayBooksSheet` |
...
3. **R1 残件の優先度判断**: A-9 / B-5 — v1.5.x パッチとして回収可能なものを判断（F-1/F-2・C-4・D-4 は完了。下記「R1 残件」参照）
...
| 項目 | 内容 | 備考 |
|------|------|------|
| D-4 ✅ | カレンダー同日複数冊の一覧 | ✅ 完了 (2026-07-07・`ca60088`) |
| F-1 / F-2 ✅ | 共有の通貨・Redis エラー | **別リポジトリ `bookbank-share`**。✅ 完了 (2026-07-09・push/デプロイ反映) |
| C-4 ✅ | 通帳ソート第2キー | ✅ 完了 (2026-07-07・`a172473`) |
| A-9 | 形態フィルター中の補完取りこ込み | 低優先・R2で顕在化・未対応 |
| B-5 | Google 価格の丸め誤差 | 低優先・部分緩和（Decimal化済み・Double入力由来の誤差余地は残る）・⬜ 維持 |
