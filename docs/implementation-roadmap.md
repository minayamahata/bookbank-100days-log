...
**N0スパイクの扱い**: 評価ハーネス・5言語テストデータ・巻数/ストップワード辞書・合否基準の確定・実本棚ベースライン計算まで**2026-07-11〜19に実施済み**。残るシリーズ正解ラベル付け（人間タスク）の完了をもって**N0をクローズし、結果メモ `docs/n0-spike-results.md` を完結状態で凍結**。R5着手時に凍結解除し、確定パラメータをそのまま使う。
...
1. **N0のクローズ（人間→AI）**: シリーズ正解ラベル付け（`tools/n0-spike/data/` の候補リスト・30分〜1時間）→ AIが指標を集計し結果メモを完結 → **N0クローズ・成果物一式をR5着手まで凍結**
2. **R3（v1.5.0）の本番浸透確認（人間）**: クラッシュ・レビュー・データ消失報告の監視。確認後に R4 着手可
