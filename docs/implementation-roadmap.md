...
作成日: 2026-07-07
更新日: 2026-07-19（**Web優先へ全面改訂・オーナー確定**: Web v1をR6直後に前倒し・R9はWeb v1後に順次化・R6にエンタイトルメント同期を追加・R4.5メモタグ新設・R5ノードはβ→本リリース方式でWeb後へ・N0はラベル付け完了後にクローズし凍結）
...
1. **R4 v1.6.0**（R3の本番浸透確認後に着手）
2. **R4.5 v1.6.x** — メモタグ T1 + UI改善（小リリース）
...
**クリティカルパス**: R3浸透確認 → R4 → R4.5 → R6 → R8（Web公開）。**以降も直列で進める**（ソロ運営で並行作業を増やさない方針・2026-07-19確定）。EM1/EM2はR6完了後ならいつでも着手可能だが、時期はWeb v1公開後の隙間で判断する（同方針）。N0スパイクはラベル付け完了をもってクローズし、成果物（ハーネス・辞書・確定基準・結果メモ）はR5着手まで凍結。
...
3. **メモタグ設計メモの確定**: `docs/memo-tagging-design.md`（ドラフト）のレビュー・(仮)項目のオーナー確定
4. **R4（v1.6.0）の着手**: 移行 Phase 1（リポジトリ抽象化）→ R4.5 → R6 → Web v1 と直列で進行
