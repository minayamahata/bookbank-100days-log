...
**設計メモは `docs/memo-tagging-design.md`**（**T0クローズ済み・承認済み・2026-08-07**）。ユーザーが**意図して本をつなげられる**軽量記法（`#タグ`）をメモに導入する。
...
| 項目 | 実装内容 |
|------|---------|
| T1: タグ記法 ✅ 完了 (2026-08-07) | `#タグ` のパース（`Utils/MemoTagParser.swift`・純関数・NLP不要＝日韓の品詞タグ非対応の影響を受けない）。正規化はノードと同じNFKC＋小文字。開始記号は半角 `#` と全角 `＃`、`#` の直前は行頭・空白・改行またはタグ直後、本体は Unicode `L*/M*/N*/_`（設計メモ 3.2節）。テスト12件 |
| T1: 入力・表示 | メモエディタの入力補助（`#` 入力で既存タグをサジェスト）・**メモ表示のタグハイライト ✅ 完了 (2026-08-07)**（`Utils/MemoTagText.swift`＋`UserBookDetailView` のメモ欄1箇所。着色のみでタップは持たせない）・書籍詳細のタグチップ行（**タップ導線はここが担う**・設計メモ 4.4節） |
| T1: タグ活用 | 本棚のタグ絞り込み（**チップ方式で確定**・設計メモ 4.1節。`normalFilterRow` の3タブは変更しない）。書籍詳細のタグチップ行と同時に実装する |
