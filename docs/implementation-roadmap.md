...
作成日: 2026-07-07
更新日: 2026-08-07（**R4完了**——移行Phase 1〈リポジトリ抽象化〉の全7ステップが終了し、実機での全画面突合と目視スモークをオーナーが完了。完了マーク・完了条件・申し送り〈設計メモ第10章〉への参照を追加し、実施順とネクストアクションを R4.5 起点へ更新。**R4.5 着手時の最初の判断は共有IDのuuid化を含めるかどうか**）／2026-07-19（**Web優先へ全面改訂・オーナー確定**: Web v1をR6直後に前倒し・R9はWeb v1後に順次化・R6にエンタイトルメント同期を追加・R4.5メモタグ新設・R5ノードはβ→本リリース方式でWeb後へ・N0はラベル付け完了後にクローズし凍結）
...
2. **R4.5 v1.6.x** — メモタグ T1 + UI改善（小リリース）。**次はここ**。着手時にR4の申し送り（設計メモ 第10章）を確認し、**共有IDのuuid化を本リリースに含めるかを最初に判断する**
...
4. **R4.5（v1.6.x）の着手**: メモタグ T1 + UI改善。**着手時にまず、共有IDのuuid化を本リリースに含めるかを判断する**（R4申し送り 10.1。前倒しを推奨・オーナー判断待ち）。あわせて `docs/memo-tagging-design.md`（ドラフト）のレビューと (仮) 項目の確定が要る
