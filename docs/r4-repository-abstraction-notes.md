...
### 10.1 最優先の判断: 共有IDのuuid化を R4.5 へ前倒しするか、R6 まで待つか

**前提6の維持根拠は失われている（8.3節-25）**。前提6は「uuidへ切り替えると同じリストが別URLになる」ことを理由に `persistentModelID` の維持を選んだ。しかしサーバ側の確認により、**現状すでに、アプリを再起動してから再共有すれば毎回別URLが発行されている**ことが分かった。したがって uuid化は「既存URLを壊す変更」ではなく、**壊れている状態を直す変更**である。R4に混ぜないことだけは確定している（挙動不変リリースのため）。

**実装コストの見積り**: `ReadingList.uuid` はR3で全行に埋まっており（`UUIDBackfillMigration`）、`ReadingListDTO.id` としてDTOにも載っている。クライアント側は `ShareService` が送る `readingListId` を `legacyShareId` から `id` へ替える1行に近い。サーバ側は `mapping:${readingListId}` をそのままキーにしているため、**キーに入る文字列が変わるだけでコード変更は不要**と読める（要確認だが、変更が要るとしても小さい）。

**3つの論点と、現時点で言えること**

- **① 切替時点で生きている共有URLの孤立 → 判断材料としては軽い**。uuid化すると `mapping:` のキーが変わるため、切替前に発行済みのURLは以後の再共有で更新されなくなり、TTL 30日で自然に消える。**ただしこれは新しい損失ではない**——現状も再起動をまたげば同じことが起きており、孤立URLは既に日常的に発生している。影響は「最大30日、古い内容のページが残る」で上限が決まっており、**切替を遅らせてもこの損失は減らない**（むしろ孤立を生み続ける期間が延びる）
- **② サーバ側 `mapping:` キーの移行 → 不要、かつ原理的に不可能。この論点は閉じてよい**。移行するには「旧キー → 新キー」の対応が要るが、旧キーは `"\(persistentModelID)"` であり**プロセスごとに別文字列**になる（8.3節-25 の実測）。したがってアプリは自分が過去に登録した旧キーを再現できず、サーバも `mapping:` の値から `uuid` を逆引きできない。**旧キーを特定する手段が双方に存在しない**ため、移行は設計として成立しない。TTL 30日での自然消滅を待つのが唯一の選択肢であり、①と同じ結論に帰着する
- **③ 「共有し直すと前のリンクも新しい内容になる」という挙動変更 → 実際に判断が要るのはこれだけ**。uuid化後はキーが安定するので、再共有すると**既存の `shareId` に上書き**され、先に配ったリンクが新しい内容を指すようになる。方向としては改善だが、副作用に注意が要る——**共有したあとにリストから本を削除しても、古いリンクが更新されて削除後の内容を見せる**（現状は古いリンクが古い内容のまま残る）。大半の利用者には期待どおりだが、「一度共有したページはそのまま」と理解していた利用者には驚きになる。**リリースノートの対象**

**推奨（オーナー判断待ち）**: **R4.5 への前倒しを推す**。(a) 実装コストが小さい（クライアント1行・サーバ変更なしの見込み）、(b) 放置するほど孤立URLを生み続ける、(c) R4.5 は元々UI改善を含む小リリースで、挙動変更を出せる枠である——の3点による。**前倒ししない理由になりうるもの**は、R4.5 の主題がメモタグT1であり、そこへ共有まわりの挙動変更を混ぜるとリリースノートと回帰テストの範囲が広がること、および共有ページ（`bookbank-share`）側の動作確認が別途必要になることである。

**前倒しする場合に決めること**: ③をリリースノートにどう書くか（例:「共有リンクを再発行せず、同じリンクの内容を更新するようになりました」）。切替後の30日間は新旧のリンクが混在することも、サポート観点で把握しておく。
