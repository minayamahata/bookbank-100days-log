...
作成日: 2026-07-11
更新日: 2026-08-07（**R4.5 ステップ1＝共有IDのuuid化を実装**——`ShareService` が `readingList.id` を送る形になり、`ReadingListDTO.legacyShareId` と `ModelDTOMapping` の該当行を削除。テストを `testShareIdIsTheUUIDAndSurvivesEdits` / `testShareIdIsStableAcrossReload` に差し替え、**8.2節が要求していた「再起動をまたいでも同一URL」を初めて満たした**。前提6・5.5節・8.2節の表・8.3節-16・8.3節-25 を解消済みへ更新し、10.1 に実装結果を追記。**アプリ本体の `persistentModelID` 参照は `ReadingListOrderMigration` のみになった**。テスト136件全通過）／2026-08-07（**共有IDのuuid化を R4.5 に含めることをオーナーが確定**——10.1 を推奨から確定した判断へ書き換え。サーバ側がID形式に依存していないことをオーナーが確認〈`isSharedList` は `readingListId` を検査対象に含めず `mapping:` のキー文字列として使うだけ〉、クライアントは `ShareService.swift:114` の1行、ローカルに永続化した `shareId` も無いためコストが確定した。`legacyShareId` の削除と 8.3節-16 のクローズを同じコミットにまとめること、メモタグT1とはコミットを分けること、リリースノートは「共有リンクは常に最新の内容を表示します」に統一することを明記。スナップショット共有を 10.2 に将来の検討項目として追加）／2026-08-07（**ステップ7完了＝R4完了**——実機でのスクリーンショット突合と目視スモークをオーナーが実施し、**7.2 のB枠は全画面でベースラインと一致・A枠3件の差分も想定内・C枠の動きも異常なし**（C-5 の共有URLは 8.3節-25 の既知不具合として意図的にスキップ）。結果を 7.2 と 8.3節-12 へ記録／ロードマップR4に完了マーク／ステータス行と7章ステップ6・7の行、8.4節の人間タスク欄のズレを修正／**新設 第10章にR4完了後の申し送りを集約**——最優先の判断は共有IDのuuid化を R4.5 へ前倒しするか R6 まで待つかで、8.3節-25 の3論点に踏み込んだ検討〈②のサーバ側 `mapping:` キー移行は**不要かつ原理的に不可能**という結論を含む〉を添えた）／2026-08-06（**ステップ7の1コミット目＝V-1 の原因確定と修正**——8.3節-12 を「原因未特定・ステップ7送り」から**原因確定・修正済み**へ全面改訂。原因は①計測ロック〈`.frame` が `.onGeometryChange` より前段のため計測対象が固定 frame 自身になる・R4着手前から存在〉と②初回フレームの口座ゼロ件〈ステップ3の `@Query` → `@State` 置換で混入〉の**合成**で、根拠は「コード上の機構特定」＋「②を埋める修正のみで症状消失（実機確認）」の2段構成。修正は**A＝データ側のみ**〈`allPassbooks` に `latestSnapshot` フォールバック。理由は同一ファイル内の非対称の是正——`allUserBooks` は既に同型のフォールバックを持っていた〉で、①は症状を出さない状態で温存〈将来 `accountInfoSection` の高さが後から変わる要因が増えれば同型で再発しうる〉。`latestSnapshot` 非空の保証は `MainTabView` のゲートが発火済みであることからの導出として明記。**未実施の T-D / T-A**〈修正前ビルドでのみ観測可能・観測窓が閉じたため。「②を埋めたら消えた」が両者より強い証拠〉と、**T-B が5秒スプラッシュで観測不能**〈`BookBankApp.swift:219`。観測できれば②の直接証拠だが観測できないことは反証にならない＝片側のみ有効な判別軸〉も記録）／2026-08-06（**ステップ6の3コミット目＝ステップ6完了**——8.3節-25 の実害を「未検証」から**検証済み・実害あり**へ更新〈オーナーがサーバ側 `bookbank-share/app/api/lists/route.ts:82-90` を確認。`mapping:${readingListId}` にヒットしなければ `nanoid()` で新規発行するため、**再起動をまたいだ再共有は毎回別URLを発行し、先に配ったURLはTTL30日のあいだ古い内容で残る**。R4の後退ではない既存の不具合で、R4では修正しない〉／同項に申し送りとして**前提6の維持根拠が実質失われたこと**と、uuid化を R6 まで待つか R4.5 に前倒しするかを別途判断すべき論点を明記／`ReadingList.saveBookOrder(_:)` を 8.3節-26 の基準で削除〈オーナー指摘。8.5節が `orderedBooks` と1行で対にしている〉）／2026-08-06（**ステップ6の2コミット目**——`@Model` の計算プロパティのうちDTOへ移設済みの9件を削除し、基準外〈DTOに等価物が無く単に未使用〉の8件は理由を明記して残した。`ReadingList.orderedBooks` と `Passbook.isOverall` を参照していたテスト2件は、マイグレーション／永続化が実際に書くもの〈`bookIds`・`type`〉へアサーションを向け直した。`UserBook.coverUIImage` の消滅で8.4節のクラッシュ経路が型レベルで塞がった〈新規 8.3節-26。8.3節-4 にも除外理由を追記〉）／2026-08-06（**ステップ6の1コミット目**——`primeLocalCoverCache` の static 化で 4.6節の解体対象増分を取り消し〈8.3節-22〉／`legacyShareId` の再起動またぎテストを追加したところ落ち、**共有IDが再起動をまたぐと変わるという既存の問題を検出**〈新規 8.3節-25。旧実装も同一の式でR4の後退ではないが、8.2節の「リリース前後をまたいで同一URL」は達成不能な要求だったため表と前提6を訂正〉／`Views/` の `import SwiftData` を10ファイルから除去し `AppMenuView` のみ恒久例外として据え置き〈8.4節・8.3節-2〉／プレビュー整理を完了し `PreviewSupport` の `@Model` 直接生成は構造的制約として対象外に〈8.3節-5〉／計算プロパティ削除の前提7ゲートを確認〈参照ゼロ・削除はオーナー承認待ち〉）／2026-08-06（**ステップ5検収（Claude Code / Codex の2系統）の反映**——8.4節の見出し消失を修復・「未コミット」表記と V-2 の「新規混入」表記の矛盾を解消・8.3節-17 の「最大4回」を購読者数3に整合・7章ステップ5①の「全件fetch→`#Predicate` に変更」という事実誤認を訂正／4.5節に参照解決の not-found 契約を追記／8.3節 20〜24 を追加〈エクスポート順の挙動差分・`latestSnapshot` 非空前提・ステップ6送り2件・`bookIds` 正規化がローカル専用ストア前提であることのR6制約〉／8.3節-18 の同型箇所のうち `ReadingListView` の削除アラートを前提13の範囲内で解消し、残り2件に更新／検収修正後の実機確認5項目の結果と担保範囲を7章ステップ5行と8.3節-21へ記録）／2026-08-06（8.3節-19 に作成フロー完了時のリスト名入力画面の露出＝既知UX問題を追加・8.3節-18 の「ステップ5で混入した理由」を訂正〈旧実装にも Optional 束縛はあり、外れたのは `context.save()` という偶然のマスクだった〉）／2026-08-05（ステップ5実装反映・8.3節-3 をドラフト方式で確定・8.3節 13〜18 を追加・8.3節-18 に V-2＝作成フローの真っ白画面の原因と修正、および「ログ挿入では観測できない」調査上の制約を記録・その反証を受けて 8.3節-12 の判別軸を片側のみ有効に下方修正・8.3節-17 の購読者数を4→3に訂正・8.4節のリスト系解消・4.6節の参照Viewを更新）
ステータス: **R4完了（2026-08-07）**——全7ステップ終了。`@Query` ゼロ／`Views/` から `@Model` 参照ゼロ／未使用 `modelContext` ゼロ。ユニットテスト**136件全通過**、**実機でのスクリーンショット突合と目視スモークをオーナーが完了**（7.2・前提13の「見た目・挙動を変えない」を満たすことを確認）。V-1（通帳シートの重なり・8.3節-12）は原因を確定させて修正済みで、ステップ7の突合でも再発なし。**R4完了後の申し送りは第10章に集約**（最優先は共有IDのuuid化の時期＝R4.5 か R6 か）。残る人間タスクは**リリース後の Xcode Organizer でのクラッシュ再発ゼロ確認のみ**（8.4節）。ステップ0のベースラインスクショは 2026-07-23、ステップ4は `656db5e`、ステップ5は `0745077`〜`b72ec42`、ステップ6は `bb9e732`〜`dc0fa0f`、ステップ7は `2c5a49e`〜
関連文書:
...
| 6 | 共有ID（`ShareService`） | `ReadingListDTO` に `legacyShareId: String`（`persistentModelID` の文字列表現）を運び、共有URLの同一性を維持する。uuidへの切り替えはR6で判断（R3メモ 前提7の継続）。**⚠ 2026-08-06 実測: この文字列は再起動をまたぐと変わる（`NSManagedObjectID` のポインタ値を含むため）。旧実装も同一の式なのでR4の後退ではないが、「同一性の維持」が守るのは同一プロセス内に限られる。uuid化の判断材料としてこの実測を踏まえること＝8.3節-25**。**✅ R4.5で解消（2026-08-07）**: 共有IDを `uuid` へ切り替え、`legacyShareId` は DTO ごと廃止した（10.1）。本前提は**R4期間中の記録**として残す | R3メモ 前提7 |
...
| `ReadingListDTO` | `id(uuid)` / `title` / `description` / `colorIndex` / `bookIds: [String]` / `books: [BookDTO]`（並び順解決済み・前提5）/ `createdAt` / `updatedAt` / ~~`legacyShareId: String`（前提6）~~ **← R4.5で削除（10.1）** | `books` はR4のView都合の付加物。Firestoreドキュメント自体は `bookIds` のみ持つ（R6実装が組み立てる）。`legacyShareId` も同種の付加物だったが、共有IDのuuid化で不要になり R4.5 で消えた |
...
- `ShareService` だけは例外で、`legacyShareId`（前提6）を使い共有URLの同一性を守る。**→ R4.5で解消（2026-08-07）**: `readingList.id`（uuid）を送る形になり、例外は無くなった（10.1）
...
### 7.2 ステップ7: 突合の狙い所（変更点の一覧・2026-08-06 作成）

7.1 のベースラインと現行ビルドを突き合わせる人間タスクのための対応表。**R4は前提13（見た目・挙動を変えない）なので、原則は「全画面が一致する」が期待値**。したがって作業は「一致を確認する」ではなく、**下記A以外で差分が出たらそれが回帰である**という形で進める。

**A. 差分が出てもよい画面（承認済み挙動差分・記録済み）**

| 7.1 の # | 画面 | 出うる差分 | 記録 |
|---|------|-----------|------|
| 2 / 3 | 本棚・本棚カレンダー | `registeredAt` が**完全一致**する本どうしの相対順序が確定する（正準ソートに第2キー `createdAt` 降順が加わった）。同秒登録の本が無ければ差は出ない | 8.3節-10 |
| 9 | 口座編集のエクスポートプレビュー | サンプル4冊と詳細サンプル1冊が「上が新しい」順に確定（旧は SwiftData 既定順＝不定） | 8.3節-8 |
| 11 | リスト詳細のエクスポートプレビュー | 同型。旧はリレーション順（不定）、新は**画面の表示順**（`bookIds` 順） | 8.3節-20 |

**B. 一致するのが期待値の画面（不一致なら回帰として調査）**

- **1（起動直後）**: 空状態が一瞬も瞬かないこと（5.1節）。`latestSnapshot` と `hasResolved*` ガードで潰した箇所なので、**空表示が見えたら回帰**
- **8（通帳詳細）**: V-1 の修正でベースラインと一致する方向に戻っている（8.3節-12）。ここは**シートが口座カプセル列に重なっていないことを確認する枠**として使う。並び順は旧 `@Query` と完全一致のため不変（8.3節-10）
- **4 / 5（書籍詳細・書籍編集）**: 表紙は `LocalCoverImage` 化したが `LocalCoverDataCache` への同期投入で初回フレーム差を潰しており（8.3節-9）、テーマ色も `latestSnapshot` で埋めている（8.3節-7）。**静止画は一致するはず**
- **7 / 10（通帳一覧・リスト一覧）**: `sortOrder` 昇順 / `updatedAt` 降順とも不変
- **6 / 12 / 13 / 14 / 15 / 16**: データ取得経路がリポジトリへ移っただけで表示ロジックは不変

**C. 静止画では拾えない項目（動きの目視スモーク・実機）**

1. **登録直後の即時反映とトースト**: 成功トーストと「登録済み」バッジが `await addBook` 完了後になり**1ランループ遅れる**（8.3節-11・承認済み）。「保存に失敗したのに成功トーストが出る」ことが無いのが要点
2. **リスト作成フロー**: 完了確定が1ランループ遅れる（8.3節-14）。**リスト名入力画面が約1秒露出する**のは R4着手前から存在する既知UX問題で、R4では是正しない（8.3節-19）
3. **リストのペイウォール**: 3本ある状態で4本目を作ろうとしてペイウォールが出ること、デフォルト名の連番が既存件数＋1であること（8.3節-21。オーナー先行確認済みだが「一度も fetch していない初回フレーム」は狙って作れないため項目として残す）
4. **表紙のLRU退避後の再描画**: 大量スクロール後に戻ったとき、手動登録＋写真登録の本の表紙が1フレーム出ないことがある（8.3節-9・承認済み）
5. **共有URL**: リストを共有してURLが開けること（前提6）。**アプリ再起動をまたいで再共有すると別URLになる**のは R4着手前から同じ既存不具合で、R4では修正しない（8.3節-25）
6. **削除の波及**: 口座削除で所属本も消えること（4.4節）、本の削除でリストの `bookIds` からも消えること
7. **並び順の永続**: リスト内で本を並び替えてアプリを再起動し、順序が維持されること（ユニットテストでも担保済み）
8. **空シートが出うる2箇所の点検**: `ReadingListDetailView` の共有プレビューと `BookSearchView` の手動登録（8.3節-18）。V-2 と同型の構造が残っているため、**白いシート／空のシートが出ないか**を意識して通す

**D. 判定の扱い**: Aに該当しない差分が出た場合は前提13違反として扱い、原因を特定してから先へ進む。Cの各項目は「承認済みの差分がその範囲に収まっているか」の確認であって、差分の有無そのものを問うものではない。

**実施結果（オーナー実機・2026-08-07・問題なし）**

- **B枠: 全画面でベースラインと一致**。前提13（見た目・挙動を変えない）を満たすことを確認した。#1 の起動直後に空状態の瞬きは無く、#8 の通帳詳細もシートが口座カプセル列に重なっていない（V-1 の修正がベースラインと一致する方向に戻したことの確認・8.3節-12）
- **A枠3件の差分はいずれも想定内**。本棚・カレンダーの第2キー順（8.3節-10）、口座編集とリスト詳細の各エクスポートプレビューの順序（8.3節-8・8.3節-20）とも、記録済みの承認済み挙動差分の範囲に収まっていた
- **C枠の動きの確認も異常なし**。ただし **C-5（共有URL）は 8.3節-25 の既知不具合として意図的にスキップ**した——再起動をまたいだ再共有で別URLが発行されるのは R4着手前から同じ挙動であり、R4の回帰判定の対象ではないため。uuid化の時期は第10章の申し送りで判断する
- **これをもってステップ7の検証を完了とし、R4を完了とした**
...
4. **`Passbook.bookCount` / `totalValue` 等の計算プロパティ**: リレーション経由の集計。View側は現状 `allBooks` のメモリ上フィルターで集計している箇所が主のため、DTOには**持たせず**、集計はViewの純関数に統一する（Firestoreドキュメントにもこれらのフィールドはない。`users/{uid}` のカウンターはR6のCloud Functions管轄＝移行設計書 3.3節）
   - **ステップ6での帰結（2026-08-06・オーナー判断）**: DTOに等価物を持たせない方針をここで決めているため、これらは**ステップ6の削除基準「DTOへ移設済みの計算プロパティ」に当たらない**。したがって `Passbook.bookCount` / `Passbook.totalValue` / `ReadingList.bookCount` / `ReadingList.totalValue` / `ReadingList.displayTotalValue` は**削除せず残す**（8.3節-26 の除外リスト）
...
12. **通帳シートが口座カプセル列に重なる（V-1・2026-08-05 検出・2026-08-06 原因確定・修正済み）**: 総合口座の通帳（`PassbookDetailView`）で、入金履歴シート（`PassbookDepositSheet`）が口座カプセル列（`overallAccountPassbookLinks`）に覆いかぶさる。**アプリ起動後の初回表示時に発生**。検出時点では実機ゲート（本節の 0〜2 相当＝起動・登録の即時反映・表紙）を通過済みで機能影響（保存・削除・反映）が無かったため、**ステップ4では修正せずステップ7送りとした**（オーナー判断 2026-08-05）。ステップ7の頭で調査し、原因を確定させて修正した（2026-08-06）
   ...
   - **原因（確定・2026-08-06）**: **2つの要因の合成**。どちらか一方だけでは症状にならず、両方が揃って初めて「実際より低い高さが恒久的に固定される」状態が生まれる
     - ① **高さ計測の自己参照ロック（機構・R4着手前から存在）**: `accountInfoSection` は `.frame(height: accountSectionHeight > 0 ? accountSectionHeight : nil, alignment: .top)`（`PassbookDetailView.swift:211`）を**適用した後段**に `.onGeometryChange { proxy.size.height }`（:213-219）を置いている。したがって一度 `accountSectionHeight` に値が入ると、以後の計測対象は「固定した frame 自身の高さ」になり `abs(accountSectionHeight - height) > 0.5` が二度と成立しない。内容が後から伸びても frame は clip しないためオーバーフローし、直下のシートと重なる。**①単独では無害**——正しい高さで固定されるだけ
     - ② **初回フレームの口座ゼロ件（引き金・ステップ3で混入）**: 口座が `@State private var allPassbooks: [PassbookDTO] = []` で、書籍側の `loadedUserBooks ?? repos.books.latestSnapshot` に相当する初回補完を持っていなかった。`.task` の購読は初回 body 評価より後に走るため（5.1節）、最初のフレームは `customPassbooks.isEmpty == true` で **`overallAccountPassbookLinks` ブロックが `.padding(.top, 32)` ごと存在しない**＝実際より低い高さで計測される。**②単独でも無害**——再計測されれば正しい高さに追随する
   - **確定の根拠（2段構成）**: (1) **コード上の機構特定**——`.frame` が `.onGeometryChange` より前段にあるため計測対象が固定 frame 自身になること（:211 と :213-219 の並び）と、ステップ3の `@Query` → `@State` 置換によって初回フレームの `customPassbooks` が空になったこと (2) **実測**——**初回フレームを `latestSnapshot` で埋める修正のみで症状が消失**（実機確認 2026-08-06。タブ往復後も再発しない）。①には一切手を入れていないため、②が引き金であることが実測で確定した
   - **先行 Ask セッションの仮説とのズレ**: 同セッションは②を原因とし「口座を切り替えれば解消する」と予測したが、実機(c)では解消しなかった。`MainTabView` が `PassbookDetailView` に `.id("passbook-\(displayPassbookID)")` を付けており、**口座切替では View が作り直されて `@State` が初期化される**（＝再び空配列から同じ経路を辿る）ため、そもそも切替は回復操作にならない。仮説側がこの `.id()` を見落としていた。実機(b)(c) はいずれも①②の合成という確定した理解と矛盾しない——(b) は高さが固定済みだからドラッグでは戻らず、(c) は `.id()` による作り直しで再び空配列から同じ経路を辿るため
   - **混入時期（確定）**: ①は**R4着手前から存在**（ベースラインにも同じ `.frame` → `.onGeometryChange` の並びがある）。②は**ステップ3**の `@Query` → `@State` 置換で入った。旧 `@Query` は初回 body から同期で全口座を持っていたため、初回フレームが空になる窓がそもそも存在しなかった。**ベースライン(a)で再現しないのはこのため**——「口座のストリーム化はステップ3、書籍のストリーム化はステップ4なので比較だけでは切り分けられない」と書いていた点は、②が口座側の要因と確定したことでステップ3に確定した
   - **修正（2026-08-06・修正A＝データ側のみ・オーナー判断）**: `@State private var allPassbooks: [PassbookDTO] = []` を `@State private var loadedPassbooks: [PassbookDTO]?` ＋ `private var allPassbooks: [PassbookDTO] { loadedPassbooks ?? repos.passbooks.latestSnapshot }` に置き換えた（`.task` の代入先も `loadedPassbooks` へ）。読み手は `customPassbooks`（:123）1箇所のみ
     - **理由は「同一ファイル内の非対称の是正」**: 同Viewの `allUserBooks`（修正前の :87-88）は**既に** `loadedUserBooks ?? repos.books.latestSnapshot` のフォールバックを持っており、`allPassbooks` だけが素の `@State = []` だった。8.3節-7（`EditBookView` / `UserBookDetailView` と同型）という外部との同型性より、**同じファイルの中で片方だけ欠けていた**というこちらの根拠のほうが直接的（オーナー指摘 2026-08-06）。新パターンの導入ではない
     - **`latestSnapshot` が非空である保証（ゲート自体が発火済みの証拠）**: `MainTabView.passbooks` も `@State = []`（`MainTabView.swift:68`）でフォールバックを持たない。その `passbooks` から算出した `customPassbooks.isEmpty` が `PassbookDetailView` を構築するかどうかのゲート（:267-273）になっているため、**`PassbookDetailView` が構築されたという事実自体が MainTabView 側の購読が1回以上発火し終えた証拠**であり、その発火の中で `SwiftDataPassbookRepository`（:82）が `latestSnapshot` へ代入を済ませている。さらにゲートは非空を要求するので**スナップショットの非空まで同時に保証される**。「fetch が完了しているはず」という期待ではなく構造からの導出になっている（オーナー指摘 2026-08-06）
   - **計測ロック①は残る（症状を出さない状態で温存）**: 修正Aは②を塞ぐ対症であり、①の自己参照ロックはコードにそのまま残る。初回フレームから正しい高さで固定されるため症状は出ないが、**将来 `accountInfoSection` の高さが後から変わる要因が増えれば同型で再発しうる**。その場合の筋は**計測を `.frame` 適用前へ移す**こと（背景の計測用オーバーレイ等）
     - **修正B（計測側の是正＝`customPassbooks` の件数変化で `accountSectionHeight` をリセット）を採らなかった理由**: 修正Aを入れると初回フレームから正しい `customPassbooks` が入るため、後続でストリームが届いても内容が同一で `.onChange` は発火しない。**追加コードはほぼ不活性**で、前提13（見た目・挙動を変えない）に対する1フレーム分のリスクだけが残る（オーナー判断 2026-08-06）。修正D（ステップ7以降への再送り）も、原因確定済みかつ修正が同一ファイル内の既存パターンの適用なので送る理由がない
   - **ステップ7の突合でも再発なし（2026-08-07）**: 7.2 の実機突合で、通帳詳細（7.1 の #8）はベースラインと一致し、**シートが口座カプセル列に重なっていないことを確認**した。修正の効果が単発の観察ではなく突合の枠内で裏付けられたため、本項は完了として閉じる（残る留意点は上記「計測ロック①は残る」のみ）
   - **トレードオフ**: **4.6節の暫定配置（`latestSnapshot`＝R6で解体予定）を口座側へさらに広げた**ため、R6の解体対象が1箇所増える。8.3節-7（起動直後の口座色）・8.3節-9（LRU退避後の表紙1フレーム）と同根の「**初回フレームにデータが無い**」問題であり、`latestSnapshot` はこの系統に対する共通の当て木として機能している
   ...
   - **判別軸は結局適用しなかった（T-D / T-A 未実施・2026-08-06）**: 修正前ビルドでのみ観測できる次の2件は**実施していない**。実機が修正Aを含むビルドへ更新された時点で観測窓が閉じたため（重なりは解消し、タブ往復後も再発しないことを確認）
     - **T-D（判別軸の適用）**: 重なった状態でアプリスイッチャーのジェスチャにより再レイアウトを強制し、解消するかを見る。予測は「解消しない」（`accountSectionHeight` は固定 frame 自身を測り直すだけなので更新されない）。解消した場合は `accountSectionHeight` の反映順序という第3の筋が残り、②には絞り込めなかった
     - **T-A（ロックの持続性）**: 本棚タブへ移動して通帳タブへ戻り、重なりが残るかを見る。予測は「重なる」（`TabView` はタブ切替で子を破棄しないので `@State` の `accountSectionHeight` はロックされたまま維持される。また `selectedTab` の既定値は 1＝通帳タブなので `PassbookDetailView` は起動時点で構築され、ロックが確定する最初のフレームは5秒スプラッシュに覆われてユーザーには見えない）
     - **やり直しは不要と判断（オーナー判断 2026-08-06）**: **「②を埋めたら症状が消えた」という実測は、T-D / T-A が与える間接的な裏取りより強い証拠**である。両テストは①②の切り分けを補強するだけで結論を変えない
   - **T-B（登録ボタンの一拍遅れ）は5秒スプラッシュで観測不能——片側のみ有効な判別軸（2026-08-06）**: ②の直接の可視化として「起動直後の総合口座で、丸い『本を登録』ボタンとカプセル列が一拍遅れて現れる」を観測する案があった。`registrationPassbook` は `passbook ?? customPassbooks.first`（:181-183）なので `customPassbooks` が空の間は `nil` になり、`if let registrationPassbook` で包まれた登録ボタン（:537-542）とツールバーの `+`（:325）が**そのフレームには存在しない**——①では説明できない②固有の症状であり、観測できれば②の直接証拠になる
     - **しかし観測できない**: `RootView` のスプラッシュは `DispatchQueue.main.asyncAfter(deadline: .now() + 5.0)`（`BookBankApp.swift:219`）で `MainTabView` を**5秒間**覆う。`PassbookDetailView` の構築からストリーム到達までの1ランループはその中に完全に埋まるため、**スプラッシュが明けた時点では登録ボタンもカプセル列もすでに揃っており、裏で `accountSectionHeight` のロックだけが確定している**
     - **したがって「揃っている＝②の反証」は成立しない（偽陰性）**。本項の「ただしこの判別軸は片側しか使えない」で記録した「直らない＝データ欠落系の側だけが有効」と同じく、**片側のみ有効な判別軸**として扱う: 観測できれば②の直接証拠だが、**観測できないことは②の反証にならない**（オーナー指摘 2026-08-06）
     - **副産物**: `SplashPresentationState.hasScheduledInitialSplash` はプロセス単位の静的フラグなので初回起動では必ず5秒入る。つまり②が実機に残す観測可能な痕跡は**ロックされた高さ＝重なりそのものだけ**であり、コードによる計測（`print` は 8.3節-18 のとおりこの系統では使えない）を入れない限り、V-1 の観測集合はここで閉じていた
...
16. **ドラフト中の `ReadingListDTO` は `legacyShareId` が空文字（ステップ5・2026-08-05）→ ✅ R4.5でクローズ（2026-08-07）**: 共有IDのuuid化に伴い `legacyShareId` を DTO ごと削除したため（10.1）、**この落とし穴はフィールドごと消滅した**。ドラフトDTOの `id` は `UUID().uuidString` で最初から確定しており、永続化の前後で値が変わらない。以下はR4期間中の記録: `AddReadingListView.startBookSelection()` が組み立てるドラフトDTOは `legacyShareId: ""` で、実際の値は `addReadingList` で永続化された後に `ModelDTOMapping.readingListDTO(from:)` が `"\(model.persistentModelID)"` から再解決する。したがって**ドラフトDTOを `ShareService.shareReadingList` へ渡すと空の共有IDでリクエストが飛ぶ**。ステップ5時点の作成フローに共有への導線は無いため前提6（共有URLの同一性）には影響しないが、**「まだ永続化されていない＝共有IDが未確定のDTO」を型が表現していないため、この取り違えは実行時まで検出できない**。R6でDTOをレイヤーを跨いで扱う（ローカル→Firestore、未同期→同期済み）際の注意点として残す
...
25. **共有IDは再起動をまたいで変わる（ステップ6で実測・R4着手前から同じ・2026-08-06・8.3節-23 の帰結）→ ✅ R4.5で修正済み（2026-08-07・10.1）**: 共有IDを `uuid` へ切り替えたことで解消した。`ShareService` は `readingList.id` を送り、`legacyShareId` は DTO ごと削除。再起動をまたいだ再共有でも `mapping:${uuid}` が同じキーを引くため**同じURLの内容が更新される**。以下は問題の記録: 8.3節-23 のテストを書いたところ落ちた。`legacyShareId`（＝`"\(persistentModelID)"`）は**同じレコードでもプロセスごとに別文字列になる**。実測値（同一ストアを2つのコンテナで順に開いた場合）:
    ...
    - **実害（オーナーがサーバ側を確認・2026-08-06・検証済み）**: `bookbank-share/app/api/lists/route.ts:82-90` が `mapping:${readingListId}` をキーに既存 `shareId` を引き当て、**ヒットしなければ `nanoid()` で新規発行する**。したがって次が確定した帰結である。
      - **アプリの再起動をまたいで同じリストを再共有すると、毎回別の共有URLが発行される**
      - **先に配ったURLは内容が更新されない**（`list` / `mapping` とも TTL 30日で存続するため、古い内容のページが1か月生き続ける）
      - **R4の後退ではなく、旧実装から存在した既存の不具合**である（算出式が同一のため）
    - **R4では修正しない（オーナー判断 2026-08-06）**: R4は挙動不変リリースであり（前提13）、共有IDの算出を変えることは**既存の共有URLの扱いを変える＝挙動変更**に当たる。R4は本項の記録までにとどめ、ステップ7へ進む
    - **⚠ 申し送り: 前提6の維持根拠は実質失われた（uuid化の時期を別途判断すること）**。前提6は「uuidへ切り替えると同じリストが別URLになる」ことを理由に `persistentModelID` の維持を選んだ。しかしサーバ側の確認により、**再起動をまたげば現状すでに再共有のたびに別URLが発行されている**ことが分かった。つまり「維持すれば同一URLが保たれる」という前提自体が成り立っておらず、**uuid化は既存URLを壊すどころか、壊れている状態を直す変更**である。`ReadingList.uuid` はR3で既に全行に埋まっており（`UUIDBackfillMigration`）、`ReadingListDTO.id` としてDTOにも載っているため、実装コストは `ShareService` の1行に近い
      - **判断が必要なこと: uuid化を R6 まで待つか、R4.5 として前倒しするか**。R4に混ぜないのは確定（挙動不変リリースのため）。前倒しする場合の論点は、① 切り替え時点で生きている共有URL（TTL 30日）が孤立すること ② サーバ側の `mapping:` キーの移行が要るか ③ uuid化後は再起動をまたいでも同じURLに上書きされるため、**ユーザーから見て「共有し直すと前のリンクも新しい内容になる」という挙動変更**になること（現状は別リンクが増える）。この③は改善だが挙動変更なので、リリースノートの対象になる
...
26. **`@Model` の計算プロパティ・メソッドの削除（ステップ6・2026-08-06・オーナー承認済み）**: 削除基準は「**DTOへ移設済み**のもの」であり、単に未使用であることは基準ではない。前提7ゲート（マイグレーション3種＋`StoreBackupManager` からの参照ゼロ）は確認済みで、候補すべてについて参照ゼロだった
    - **削除した10件**——いずれもDTO側に等価物があり、削除後も**アプリ・テストの両ターゲットがコンパイルを通った**（＝参照ゼロの機械的な証明）:
      - `UserBook.displayAuthor` / `coverImageURL` / `hasCoverImage` / `storedCurrency` → `BookDTO` の同名メンバー
      - `UserBook.displayAmount(in:exchangeRates:)` と `Collection where Element == UserBook` の `totalDisplayAmount(in:exchangeRates:)` → `BookDTO` 側の同名。全18箇所の呼び出しは既に `[BookDTO]` に対するもので、`@Model` 版は誰も呼んでいなかった
      - `UserBook.coverUIImage` → 前提4により画像バイナリはDTOに載せないため、等価物は `LocalCoverImage`＋`LocalCoverDataCache`（8.4節）。**これが 8.4節のクラッシュ経路の入口だったプロパティであり、型から消えたことで再発が構造的に不可能になった**
      - `ReadingList.orderedBooks` → `ReadingListDTO.books`（並び順解決済み・前提5）＋ `Repositories/ReadingListOrdering.swift`（純関数）
      - `ReadingList.saveBookOrder(_:)` → `ReadingListDTO.bookIds` / `replacingBooks(_:updatedAt:)`＋`updateReadingList`（並び替えの保存経路は `ReorderBooksView` → リポジトリに移った）。**計算プロパティではなくメソッドだが、本表 8.5節が `orderedBooks` と1行で対にしている**とおり移設先は同じで、同じ基準を満たす（オーナー指摘 2026-08-06）。残っていた言及は `ReadingListOrderMigration.swift:106` とテストのコメント2箇所だけで、呼び出しはゼロだった
      - `Passbook.isOverall` → `PassbookDTO.isOverall`
      - 併せて `UserBook.swift` の `import UIKit` も不要になったため削除（`UIImage` の唯一の用途が `coverUIImage` だった）
    - **除外したもの（オーナー判断 2026-08-06）**: 削除基準に当たらないため**残す**。
      - `Subscription.isProActive` / `isFree`: `SubscriptionDTO` は存在せず、DTOへの移設が起きていない。**単なる未使用コードであり、`Subscription` の廃止はR6スコープ**（スコープ外の定義＝本書 :58）。R4で消すなら別件として理由を立てる必要がある
      - `Passbook.bookCount` / `Passbook.totalValue` / `ReadingList.bookCount` / `ReadingList.totalValue` / `ReadingList.displayTotalValue`: 8.3節-4 が「DTOには持たせず集計はViewの純関数に統一する」と決めているため、DTO側に等価物が無い。`displayTotalValue` は `totalValue` の書式化派生なので同じ扱いとした
      - `UserBook.displayPrice` / `hasISBN13`: DTO側に等価物が無い。`displayPrice` は自身のコメントどおり**R4以前から `FormattedPriceText` に取って代わられたレガシー**で、DTO移設によって不要になったものではない。上記 `Subscription` と同じ「未使用だが基準外」の分類
    - **テストの書き換え2件**: 削除した2プロパティを参照していたアサーションを、**マイグレーションや永続化が実際に書くもの**へ向け直した。`ReadingListOrderMigrationTests.migrationTranscribesLegacyOrderToBookIds` は `orderedBooks` の並びを見る行を落とし、直前の `bookIds` の直接assertに一本化（マイグレーションが書くのは `bookIds` であり、それを解決した並びは `ReadingListOrdering` 側のテストが担保する）。`DataPersistenceTests.testCreateOverallPassbook` は `isOverall` の行を落とし、直前の `type == .overall`（永続化される値そのもの）に一本化した。**どちらも冗長な二重assertを外しただけで、担保している軸は減っていない**
...
| `orderedBooks` / `saveBookOrder` | `Models/ReadingList.swift`（R3でuuid化済み） | R3メモ 5.2節 | 解決ロジックをリポジトリ実装内の純関数へ移設。DTOは解決済み `books` を内包（前提5）。**✅ ステップ6で両者を削除済み（2026-08-06・8.3節-26）**——解決は `ReadingListOrdering`、並び順の保存は `ReadingListDTO.bookIds` / `replacingBooks` ＋ `updateReadingList` が担う |
...
## 10. R4完了後の申し送り（2026-08-07 集約）

R4は完了したが、スコープ規律（前提13＝見た目・挙動を変えない／前提1・7＝触らない領域）によって意図的に手を付けなかった論点が残っている。本章はそれらを1箇所に集めたもので、**次のセッションで判断できる形にしておくこと**を目的とする。個々の詳細は 8.3節の各項目が正であり、ここには判断に必要な要約と論点だけを置く。

### 10.1 共有IDのuuid化は R4.5 に含める（✅ オーナー確定・2026-08-07 → **✅ 実装完了 2026-08-07**）

**実装結果（R4.5 ステップ1・2026-08-07）**: 計画どおり1コミットで完了した。`ShareService.swift` が `readingList.id` を送る形になり、`ReadingListDTO.legacyShareId` / `ModelDTOMapping` の該当行 / 呼び出し側4箇所（`AddReadingListView` のドラフト・`PreviewSupport`・テストのDTO組み立て5箇所）を削除。テストは `testShareIdIsTheUUIDAndSurvivesEdits` と `testShareIdIsStableAcrossReload` に差し替え、**後者は「共有IDの文字列そのものが再起動をまたいで一致すること」を固定する**（R4時点は内側のURIだけ比較する回避だった）。ユニットテスト136件全通過。あわせて 前提6・5.5節・8.2節の表・8.3節-16・8.3節-25 を解消済みへ更新した。**アプリ本体の `persistentModelID` 参照は `ReadingListOrderMigration`（前提7・一度きりの移行）のみになった**。

**決定**: uuid化は **R4.5（v1.6.x）で実施する**。R4には混ぜない（挙動不変リリースのため）。**メモタグT1とはコミットを分ける**——回帰の切り分けとリリースノートの記述が楽になるため（オーナー指示）。

**決定の根拠**: 前提6は「uuidへ切り替えると同じリストが別URLになる」ことを理由に `persistentModelID` の維持を選んだが、サーバ側の確認により**現状すでに、アプリを再起動してから再共有すれば毎回別URLが発行されている**ことが分かった（8.3節-25）。したがって uuid化は既存URLを壊す変更ではなく、**壊れている状態を直す変更**である。加えて**待つことに利益がない**——孤立URLは切替で新たに生じるものではなく現状すでに発生しているため、R6まで待つとその間ずっと「再共有のたびに別リンクが増え、先に配ったリンクは30日間古いまま」を出荷し続けることになる。

**確認済みの事実（実装コストの確定）**

- **サーバ側はID形式に依存していない（オーナー確認 2026-08-07）**: `bookbank-share/app/api/lists/route.ts` の `isSharedList`（:21-42）は `readingListId` を検査対象に含めておらず、:82 の `!= null && !== ""` を通ったあとは `mapping:` のキー文字列として使われるだけである。長さ制限・文字種チェックとも存在しない。**サーバ側の変更は不要**
- **クライアント側は1行**: `ShareService.swift:114` の `let readingListId = readingList.legacyShareId` を `readingList.id` に替える。`ReadingList.uuid` はR3で全行に埋まっており（`UUIDBackfillMigration`）、`ReadingListDTO.id` としてDTOにも載っている
- **クライアント側の移行処理は不要**: `shareId` やURLをローカルに永続化している箇所は無い（`shareURL` は `ReadingListDetailView` の `@State` のみで、シートを閉じれば破棄される）

**同じコミットにまとめる後片付け（オーナー指示 2026-08-07）**

- `ReadingListDTO.legacyShareId` フィールドと `ModelDTOMapping.swift:99`（`legacyShareId: "\(model.persistentModelID)"`）を削除する
- これにより **8.3節-16（ドラフト中DTOの `legacyShareId: ""` を型が表現していない問題）はフィールドごと消えてクローズ**する
- **アプリ本体から `persistentModelID` の使用が消える**。残るのは `ReadingListOrderMigration`（一度きりの移行・前提7で触らない）のみになる
- テストは対象が消えるため差し替える: `testLegacyShareIdMatchesPersistentModelIDAndSurvivesEdits` と `testLegacyShareIdCarriesStablePersistentIdentityAcrossReload` を削除し、**「共有IDが `uuid` と一致し、再起動をまたいでも同一である」を固定するテスト**に置き換える。8.2節が本来要求していた「リリース前後をまたいでも同一URL」は、ここで初めて満たせるようになる（8.3節-25 で「達成不能な要求だった」と訂正した記述も、uuid化の完了時点で更新すること）
- 前提6の記述を「`legacyShareId` で維持」から「uuid化済み」へ改める

**3つの論点の結着**

- **① 切替時点で生きている共有URLの孤立 → 軽い**。uuid化で `mapping:` のキーが変わるため、切替前に発行済みのURLは以後の再共有で更新されなくなり、TTL 30日で自然に消える。**新しい損失ではない**（現状も再起動をまたげば同じことが起きている）。影響は「最大30日、古い内容のページが残る」で上限が決まっており、遅らせても減らない
- **② サーバ側 `mapping:` キーの移行 → 不要かつ原理的に不可能。閉じた論点**。移行には「旧キー → 新キー」の対応が要るが、旧キーは `"\(persistentModelID)"` で**プロセスごとに別文字列**になる（8.3節-25 の実測）。アプリは自分が過去に登録した旧キーを再現できず、サーバも `mapping:` の値から `uuid` を逆引きできない。**双方に特定手段が存在しない**ため設計として成立せず、TTL 30日での自然消滅を待つ以外にない
- **③ 挙動変更 → リリースノートは「共有リンクは常に最新の内容を表示します」に統一する（オーナー確定 2026-08-07）**。uuid化後はキーが安定するので、再共有すると既存の `shareId` に上書きされ、先に配ったリンクが新しい内容を指すようになる。**共有後にリストから本を削除すると古いリンクにも反映される件は、例外や副作用として書かず、この説明から素直に導かれる挙動として扱う**。「共有した時点の内容を固定したい」という要望が出た場合は**スナップショット共有という別機能**の話になるため、10.2 に将来の検討項目として置いた

### 10.2 R4.5 の候補（小さいが挙動変更を伴うため R4 では出せなかったもの）

- **共有IDのuuid化** ✅ **実装完了**（10.1・2026-08-07・R4.5 ステップ1）
- **空のシートが出うる2箇所**（8.3節-18）: `ReadingListDetailView` の共有プレビューと `BookSearchView` の手動登録。V-2 と同型で `.sheet(item:)` 化が要る。**V-2 の教訓（偶然のマスクは無関係な変更で外れる）から、放置の危険度は当初の見立てより高い**
- **リスト作成フローの完了演出**（8.3節-19）: 約1秒の露出と、ドラフト方式で待つ理由を失った可能性のある 0.5秒 `asyncAfter`
- **スナップショット共有（将来の検討項目・現時点で要望なし）**: uuid化後の共有リンクは常に最新の内容を表示する（10.1 ③）。「共有した時点の内容を固定したい」という要望が出た場合は、既存の共有の挙動を変えるのではなく**別機能として設計する**（共有時に内容を複製して固定URLを発行する形）。要望が顕在化するまで着手しない

### 10.3 R6（クラウド移行）で必ず回収するもの

- **4.6節の暫定配置の解体**: `latestSnapshot`（V-1 の修正で口座側にも広がった・8.3節-12）と `LocalCoverDataCache`
- **`bookIds` 正規化のローカル専用ストア前提**（8.3節-24）: **正規化の1行を無検討で持ち込まないこと**。「未到達」と「削除済み」を区別できないまま正規化すると、同期到達前の本の所属を確定的に消して他端末へ伝播させる
- **パルス1回のコストが購読者数に比例する件**（8.3節-17）
- **`updatedAt` を誰が書くか**（8.3節-13）と **`EditReadingListView` のフルスナップショット上書き**（8.3節-15）: LWW競合解決の設計で一緒に回収する。**R4.7追記**: 再読の追加・編集・削除でも `updatedAt` は動かさない（お気に入り・メモと同じ）。回数の増減を LWW の「更新」とみなすかは R6 着手時に見直す
- **ドラフト中DTOの `legacyShareId` が空文字**（8.3節-16）: 型が「未確定」を表現していないため実行時まで検出できない
- **`MonthlyMemoRepository`（enum）の置換**（3.2節）・**`Subscription` の廃止**（前提1）
- **計測ロック①**（8.3節-12）: 症状は出ていないが構造は残る。`accountInfoSection` の高さが後から変わる要因が増えれば同型で再発しうる

### 10.4 リリース後の人間タスク

- **Xcode Organizer で `_KKMDBackingData.getValue` の再発ゼロを確認する**（8.4節）。`Views/` から `@Model` 参照が消え、`UserBook.coverUIImage` も型から消えたため構造的には塞がっているが、実データでの確認はリリースを経ないとできない
