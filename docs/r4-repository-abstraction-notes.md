...
26. **`@Model` の計算プロパティ削除（ステップ6・2026-08-06・オーナー承認済み）**: 削除基準は「**DTOへ移設済み**のもの」であり、単に未使用であることは基準ではない。前提7ゲート（マイグレーション3種＋`StoreBackupManager` からの参照ゼロ）は確認済みで、候補すべてについて参照ゼロだった
    - **削除した9件**——いずれもDTO側に等価物があり、削除後も**アプリ・テストの両ターゲットがコンパイルを通った**（＝参照ゼロの機械的な証明）:
