...
関連文書: `docs/cloud-migration-architecture.md`（クラウド移行設計書）/ `DESIGN_SYSTEM.md` / `docs/memo-tagging-design.md`（R4.5の `[[ ]]` 手動エッジ。T2＝R5で本書のパイプラインに合流する。0.2節方針1の補足を参照）
更新日: 2026-08-12（**R4.5からの申し送りを8.3節として追加**——つながりのラベル一覧＋リネーム・統合をR5の必須要件とする。T1が入力サジェストを廃止し「一覧で気づいて直す」方針にしたため、直す手段が本書側の担当になった）
...
| # | 方針 |
|---|------|
| 1 | 本同士のつながりは**テキストから自動生成**する。ユーザーの手動タグ付けは不要。**2026-08-10 補足（R4.5 メモつながり設計メモとの併存整理）**: 本方針は「**本を登録するだけでグラフが成立する**」というベースラインの話として維持する。R4.5で導入するメモ内の `[[ ]]` 記法（`docs/memo-tagging-design.md`）は、その上に**任意で足す手動エッジ**であり、書かなくてもグラフは成立する。自動＝機械の推測（弱い・玉石混交）、手動＝人間の宣言（強い・確実）という役割分担で、方針1を否定しない。T2（R5）での統合点は同設計メモ5章（`source: "link"` の別レーン合流・重みは順序のみ確定） |
...
> **実測注記（2026-07-11・N0）**: `NLTagger` の品詞タグ（lexicalClass）は**日本語・韓国語で非対応**（単語分割のみ対応・英語と簡体字中国語は対応）。両言語は4.2節のフォールバック（名詞抽出なし・単語分割＋ストップワード＋IDF）を常時適用する。本章の全面的な記述修正はN0完了時に行う（`docs/n0-spike-plan.md` 手順8）。
...
- **R5着手時にこの式へ「つながり共有」軸（R4.5の `[[ ]]` 手動エッジ）を追加する**。重みは順序のみ確定（シリーズ1.0 > つながり > 著者0.6）で、仕様は `docs/memo-tagging-design.md` 5章が正（2026-08-10）
- **表示ルール**:
...
### 8.3 つながりのラベル一覧とリネーム・統合（R4.5からの申し送り・2026-08-12 オーナー確定）

**R5のノードビューに「つながり（`[[ ]]` のラベル）の一覧」と、そこからラベルを*リネーム*・*統合*できる手段を入れる**。これは R4.5（メモつながりT1）から持ち込まれた**必須要件**であり、UIの磨き込み項目ではない。仕様の正は `docs/memo-tagging-design.md` 5.3節。

- **背景**: T1は入力サジェスト（既存ラベルの候補表示）を**廃止した**（同設計メモ 4.1節）。表記ゆれは「入力時に候補で防ぐ」のではなく「**一覧を見たユーザー自身が気づいて直す**」方針に切り替えたため、**直す手段が無いと方針が完結しない**
- **気づく場所はR4.5側**（本棚の絞り込みに出るつながりチップの一覧に `[[京都]]` と `[[きょうと]]` が別々に並ぶ）。**直す場所がここ**という分担
- **なぜノードビューか**: ラベルはエッジの理由であり、一覧して整えたくなる場所はグラフの側にある。絞り込みUIに編集機能を載せると役割が混ざる
- **実装の形**: つながりはスキーマを持たない（メモ本文の中の文字列）ので、リネームも統合も**該当する本のメモ本文の一括置換**になる。置換範囲は `MemoLinkParser` が返すレンジ（`[[ ]]` を含む）そのもので、本文の他の文字には触らない。統合は「キーの異なる複数のラベルを1つの綴りへ寄せる」操作（`SF`/`sf` のように正規化キーが同じものは既に同一のつながりなので対象外）
- **設計時に決めること**: ①複数冊のメモを書き換えるので取り消しか確認の一段を置くか ②R6以降はこの書き換えがクラウド同期の対象になる ③1ラベルあたりのエッジ生成上限（5章の安全弁）で**エッジからは除外されているが絞り込みには効く**ラベルも一覧に出すか
