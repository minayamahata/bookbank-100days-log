...
作成日: 2026-08-15
更新日: 2026-08-20（**並び順を最新の読書日順へ**——R4.7 再読対応の追随漏れを修正。`latestReadDate`（初回登録日と再読日の最大）降順にし、最近再読した本のメモが上部へ来るようにする。同値は `observeBooks()` の正準順を維持。既存の `ReadingTally.sortedByLatestReadDate(_:)` を再利用し、メモの有無で絞り込む**前に**全体を並べ替える。検索は並び替え済みの `memoItems` を filter するだけで再ソートしない。メモ編集日時順ではない。リポジトリの正準ソートは変更しない）／2026-08-15（**本文検索を追加**——右上の虫眼鏡から、プレーン化後のメモ本文だけを1文字ごとに絞り込む）
...
**`latestReadDate` 降順**（2026-08-20 変更。R4.7 再読対応の追随——最近再読した本のメモを上部へ）。

- `latestReadDate` は初回登録日（`registeredAt`）とすべての再読日の最大（`BookDTO.latestReadDate`）
- 再読履歴がない本は従来どおり初回登録日が基準になる
- 同値なら `observeBooks()` から渡された正準順を維持する
- 並び替えは既存の `ReadingTally.sortedByLatestReadDate(_:)` を再利用し、
  メモの有無で絞り込む**前**に `allBooks` 全体へ適用する
- 検索結果も同じ順番を維持する（filter のみ。別の基準で再ソートしない）
- **メモ編集日時順ではない**（メモ保存で `updatedAt` は動かさず、`memoUpdatedAt` 等も追加しない）
- `BookRepository` の正準ソート（`registeredAt` 降順・同値 `createdAt` 降順）自体は変更しない
...
- 検索結果の順番は `memoItems` の並び（`latestReadDate` 降順・2026-08-20〜）を維持する（再ソートしない）
