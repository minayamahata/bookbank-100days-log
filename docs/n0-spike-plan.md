...
N0はR5（ノードグラフ・v2.2.0）の前提となる検証スパイク（リリースなし・1〜2週間規模）。目的は次の2つ。
...
| ② 単語抽出・毛玉度 | IDF頻度表の目視＋統計サマリー（4.3節） | 言語別に語抽出の質を目視（特に日本語・韓国語＝品詞タグ非対応のフォールバック言語） |
...
- **フォールバック言語の語抽出品質（実測により対象拡大・2026-07-11）**: `NLTagger` の品詞タグ（lexicalClass）は**日本語・韓国語で非対応**（単語分割のみ対応）とハーネス実装時に実測で確定した。両言語は設計書 4.2節のフォールバック（名詞抽出なし・単語分割＋ストップワード＋IDF）を常時適用するため、**語抽出品質のチェック（基準5）は韓国語だけでなく日本語にも適用する**。助詞・形式名詞（「こと」「もの」「これ」等）の混入が主な検出対象
...
| 5 | **日本語・韓国語**の語抽出品質（4.3節・設計書4.2節の必須検証。品詞タグ非対応が実測確定したため両言語ともフォールバック＝単語分割＋ストップワード＋IDFを常時適用） | テストデータの各言語のタイトル・メモから抽出された語のうち**意味を成す語（名詞相当）が8割以上**。未達はまずストップワード辞書の増強（第7章）で改善し、それでも未達なら方式レベルの問題として報告 | フォールバックが「予定した縮退」から「常時適用」に変わったため、その品質自体を合否対象に格上げ（2026-07-11 オーナー承認）。助詞・形式名詞の混入が主な検出対象 |
