...
**R4.7（v1.7.0）— 再読履歴。実装完了（2026-08-18）。** 設計書 `docs/reread-history-spec.md`。実機での旧スキーマ→新スキーマ移行確認、5言語訳レビュー、受け入れは ✅ 完了（2026-08-19 オーナー確認）。**同日: データ安全性**（R4.7専用の移行前バックアップ・日付整合・編集の一括保存）。次の大型リリースは R6。
