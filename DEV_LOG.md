...
- **R4.6（v1.6.x UIパッチ枠）を新設（2026-08-13）**——R6までの合間に発生する割り込みUI改善の置き場（直列計画の外の随時枠。入場条件などはロードマップ R4.6）。1件目の**書籍詳細のヘッダー吸い付き型シートは実装済み**。**2026-08-15: 個別に作った引用が保存後に1枠へ合体する問題を修正**（空の `p.` を落とすときに、直後が別引用なら空行を境界として残す）。**2026-08-15: メモ一覧ページ（メモのノートページ）を実装**——総合口座の「口座管理」を「メモ一覧」へ差し替え、プレーンテキストのカードから本の詳細へ辿れる（設計メモ `docs/memo-list-design.md`）。候補として残るのは「スクリーンショットの画像候補」（仕様未定・着手前に設計メモを起こす）。**ウィジェットはこの枠に入れず R6完了後**（App Groupのデータ受け渡しがFirestore移行で作り直しになるため）
