...
- ✨ **新機能のお知らせを実装（v1.7.0 build 9 のリリース前UI）**
  ...
  - 同日フィードバック第2回——ライトモードのカード背景を `#ECECEC`（View限定色）、説明は行間+4で文字色 `.primary`、カード上辺32pt・説明と画像の間24pt。**実機チェック用に `temporarilyAlwaysAutoShow = true` の一時スイッチを追加**（確認済み・3回上限を無視して毎起動表示。回数は加算しない）。**リリース前に false へ戻すのが必須の人間タスク**
