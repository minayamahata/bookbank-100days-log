...
- ✅ **R4.5「メモつながり」T1の実装（ステップ5'〜9'・2026-08-11〜13）**
  ...
  - **ステップ7'（8/11〜12）: メモ編集ツールバー** — メモ編集画面の下に「つなぐ・太字・引用・ページ番号・書式クリア・ひとつ戻す」の6ボタンを新設（5言語対応）。編集中の画面でも記号を見せず、装飾がその場で反映される方式に差し替えた。実機確認を6回重ね、ボタンのトグル化・いまいる装飾のボタンが光るアクティブ表示・隠した記号をまたぐカーソルの規則の一本化など、指摘を反映して作り込んだ
  ...
  - **ステップ9'（8/13）: 検収** — 設計メモ・ロードマップ・本棚内検索仕様書の記述を実装後の姿に同期。既存メモに `[[ ]]` などが偶然含まれて意図しない装飾にならないかを実データで調べるチェックスクリプト（`tools/memo-markup-check.py`）を用意した。**実機からのデータ書き出しとチェックの実行、実機確認の残項目は人間タスクとして残っている**
