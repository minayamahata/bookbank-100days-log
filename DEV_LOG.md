...
- **R4.6（v1.6.x UIパッチ枠）を新設（2026-08-13）**——R6までの合間に発生する割り込みUI改善の置き場（直列計画の外の随時枠。入場条件などはロードマップ R4.6）。1件目の**書籍詳細のヘッダー吸い付き型シートは実装済み**（実機での見た目・操作感の確認と `book.delete.action` の訳レビューが人間タスクとして残る）。**2026-08-15: 個別に作った引用が保存後に1枠へ合体する問題を修正**（空の `p.` を落とすときに、直後が別引用なら空行を境界として残す）。候補として「メモのノートページ」「スクリーンショットの画像候補」を登録（いずれも仕様未定・着手前に設計メモを起こす）。**ウィジェットはこの枠に入れず R6完了後**（App Groupのデータ受け渡しがFirestore移行で作り直しになるため）
