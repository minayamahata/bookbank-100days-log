...
- **R4.6（v1.6.x UIパッチ枠）を新設（2026-08-13）**——R6までの合間に発生する割り込みUI改善の置き場（直列計画の外の随時枠。入場条件などはロードマップ R4.6）。1件目の**書籍詳細のヘッダー吸い付き型シートは実装済み**。**2026-08-15: 個別に作った引用が保存後に1枠へ合体する問題を修正**。**2026-08-15: メモ一覧ページ（メモのノートページ）を実装**。**2026-08-16: スクリーンショットの画像候補（マンスリーログ共有）を実装**——カレンダー表示中のスクリーンショット、または月ヘッダーの共有ボタンから透明PNG 4種をコピー／保存／共有できる。同日の完了前補正で、カレンダーが実際に前面のときだけ自動表示し、表紙準備はキャッシュ優先＋同一URLの最大4件並列、共有セルは2:3にした（設計メモ `docs/monthly-log-share-design.md`）。**同日: アプリ内タイポグラフィを LINE Seed へ全面移行**（設計メモ `docs/typography-design.md`）。**同日: マンスリーログ共有の外部PNGを描画範囲＋24pxへクロップ**（内部マスターは1080×1920のまま）。R4.6の登録項目は一通り実装済み。**ウィジェットはこの枠に入れず R6完了後**（App Groupのデータ受け渡しがFirestore移行で作り直しになるため）
