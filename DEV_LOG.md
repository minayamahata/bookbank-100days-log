...
**R4.7（v1.7.0）— 再読履歴。実装完了（2026-08-18）。** 設計書 `docs/reread-history-spec.md`。実機での旧スキーマ→新スキーマ移行確認は ✅ 完了（2026-08-19 オーナー確認）。**同日: データ安全性**（R4.7専用の移行前バックアップ・日付整合・編集の一括保存）。残る人間タスクは5言語訳レビューと受け入れ。
...
- ✅ **R4.7 の実機更新確認をクローズ（オーナー確認）**
  既存データを残したまま新版を上書き起動し、問題なし。旧スキーマ→新スキーマの移行確認は完了。残る人間タスクは5言語訳レビューと受け入れ。
...
- ✅ **R4.7 / v1.7.0 再読履歴を実装完了**
  ...
  - **人間タスク**: 新規文字列5言語の訳レビュー、受け入れ。実機移行確認は ✅ 完了（2026-08-19）
